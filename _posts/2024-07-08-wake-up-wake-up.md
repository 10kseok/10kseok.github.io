---
layout: post
title: (Writing) 일어나야해! 넌 조선의 자존심이야! (1/2)
...
### 백엔드로 이전
이 때 초기 서비스 기획 의도를 다시 떠올렸다. 사용자의 흐름에서 우리의 서비스가 자연스럽게 녹아드는 것이 목표였다. 타사의 영상을 가져와서 보여주는 것은 기획 의도와 다른 길이였고 마치 서비스가 동영상 플레이어가 되는 것만 같았다. 여러번의 코치님의 회유에도 이것만큼은 지켜야겠다고 생각을 했고 팀원들의 의견도 모두 일치했다. 졸음 감지를 백엔드로 이전하기로 결론이 났고 백엔드로 이전하면 원래 기획했던 UX로 돌아올거란 기대감에 부풀었다.
...
>우리 반의 수강생들 반응도 좋았고 최종 발표때도 이 부분에서 현장 반응이 굉장히 좋았다. 나는 발표 영상 제작과 기능 구현한다고 여러번 봤지만 볼 때마다 참 웃기다

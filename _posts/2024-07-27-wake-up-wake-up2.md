...
이전 프로젝트에서는 CI/CD를 구성하고 진행한 적이 있다. 반면에 이번 프로젝트에서는 필요성에 대한 의문이 들어 아예 없이 진행했다. 우선 테스트 코드를 작성해본 경험은 나밖에 없었고, 이런 상황에서 일정에 맞춰서 기능을 구현하기에는 빠듯해보였다. 그리하여 CI에 필요성을 못 느꼈다. 물론 그러다가 병합한 뒤 패키지 오류나 자잘한 오류를 몇번 겪긴 했으나 일정에 지장을 줄 정도는 아니였다.

진정한 CI는 하나의 메인 브랜치에 자주 병합시키는 것이라는 글을 본 적이 있다. 이 방법을 적용해도 괜찬을 것 같은 판단이 든 게, 개발하는 시간에는 항상 5명이 같이 있기에 충돌시 바로 붙어서 해결해줄 수 있는 상황이였다. 몇몇 팀원은 main에 바로 작업하는 분도 있었으나, 나는 아직 거기까진 무서워서 기능 브랜치 정도는 만들고 병합하는 식으로 개발했다.
